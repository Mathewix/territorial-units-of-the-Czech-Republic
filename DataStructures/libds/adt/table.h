...
namespace ds::adt {
    ...
    void GeneralBinarySearchTree<K, T, ItemType>::insert(const K& key, T data)
    {
        ...
        else
        {
            ...
            {
                throw structure_error("Table contains this key already.");
            }
    ...
    T GeneralBinarySearchTree<K, T, ItemType>::remove(const K& key)
    {
        ...
        if (!tryFindNodeWithKey(key, nodeWithKey)) {
            throw structure_error("No key in table like this exists");
        }
        T result = nodeWithKey->data_.data_;
        //univerzalna cast na tejto urovni
    ...
    void GeneralBinarySearchTree<K, T, ItemType>::removeNode(BSTNodeType* node)
    {
        ...
        case 0:
            ...
            break;
        case 1:
            syn = this->getHierarchy()->hasLeftSon(*node) ?
                node->left_ :
                node->right_;
            if (node->left_ == syn) {
            ...
            if (this->getHierarchy()->isRoot(*node)) {
                ...
                if (parent->left_ == node) {
                    this->getHierarchy()->removeLeftSon(*parent);
                    this->getHierarchy()->changeLeftSon(*parent, syn);
                }
                else {
                    this->getHierarchy()->removeRightSon(*parent);
                    this->getHierarchy()->changeRightSon(*parent, syn);
                }
            }
            break;
        case 2:
            BSTNodeType * predecesorInOrder = node->left_;
            while (this->getHierarchy()->hasRightSon(*predecesorInOrder)) {
                predecesorInOrder = predecesorInOrder->right_;
            }
            std::swap(node->data_, predecesorInOrder->data_);
            this->removeNode(predecesorInOrder);
            break;
        }
    }
